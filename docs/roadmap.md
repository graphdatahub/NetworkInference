...
3. Scalar node state: What if we cannot separate node states into single scalar variables $x_i(t)$ (that can be analyzed individually). Power system nodes (buses, generators) have multiple state variables (voltage magnitude, angle, frequency, generator rotor angle/speed) that can all shape the process we are studying.
   + For this, develop a tensor-based approach: Extend the framework to tensor-valued data that preserve the relationships between different state variables at each node. See the book "A Geometrical Introduction to Tensor Calculus" by Jeroen Tromp, and the corresponding course in Fall 2025, APC 588.
5. How to leverage structurial prior knowledge (approximate edge weights, flow estimations and/or known eigenvectors), which can help as initial guess, regularization and/or spectral constraint?
6. Data integration:
...
7. Processes underlying the data (assumed):
   ...
   + How to extend models to non-linear, advection-like, or interference processes (e.g., frequency/voltage disturbances over power grid)—hence learning hidden geometry under different kernel-level assumption—?
   + How to discriminate direct from indirect dependencies in those non-linear cases (since in linear cases we could attribute non-linear dependencies to indirect effects)? The goal is not node embedding but graph inference: we dont want to extract general coupling strengths, or arrival times, described as distance in an embedding space, we want to recover actual node-to-node dependencies with the goal to find a meaningful basis to the frequency domain.
